...
class AuthSellerViewSet(viewsets.ViewSet):
    ...
    def get_owned_products(self, request, *args, **kwargs):
        ...
        pk = self.kwargs['pk']
        products = Product.objects.filter(seller=self.kwargs.get('pk'))
...
class CartViewSet(viewsets.ViewSet):
    ...
    def update_item(self, request):
        ...
        item = OrderItem.objects.get(pk=get_item)        
...
class AdminUserViewSet(viewsets.ViewSet):
    ...
    def admin_get_ordered_products(self, request):
        ...
        get_order = Order.objects.get(buyer=int(user))
...
class AuthBuyerViewSet(viewsets.ViewSet):
    ...
    def get_ordered_products(self, request):
        ...
        get_order = Order.objects.get(buyer=request.user, is_completed=False)
        order = OrderSerializer(get_order)
        get_item = OrderItem.objects.filter(order=order.data['id'])
        if get_item:
            item = OrderItemCartSerializer(get_item, many=True)
